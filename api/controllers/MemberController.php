...
class MemberController {
    ...
    public function delete($id) {
        ...
        try {
            ...
            $appStmt = $this->db->prepare("SELECT id, status FROM appointments WHERE member_id = ? AND status = 'scheduled' AND ends_at > ? FOR UPDATE");
            $appStmt->execute([$id, $nowStr]);
            $appsToCancel = $appStmt->fetchAll(\PDO::FETCH_ASSOC);

            if (!empty($appsToCancel)) {
                $cancelReason = "Üyenin sistemden kaldırılması";
            ...
            if (!empty($appsToCancel)) {
                foreach ($appsToCancel as $app) {
                    try {
                        ...
                        error_log('AuditLogger failed during member delete appointment cascade: ' . $e->getMessage());
                    }
                }
            }
