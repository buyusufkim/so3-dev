...
if (isset($routes[$method][$requestUri])) {
    ...
    if (preg_match('#^/api/admin/members/([1-9]\d*)/progress-notes$#', $requestUri, $matches)) {
        AuthMiddleware::handle();
        $id = (int)$matches[1];
        $controller = new \Controllers\MemberProgressNoteController();
        if ($method === 'GET') {
            $controller->index($id);
            $matched = true;
        } elseif ($method === 'POST') {
    ...
    if (preg_match('#^/api/admin/member-progress-notes/([1-9]\d*)(/restore)?$#', $requestUri, $matches)) {
        AuthMiddleware::handle();
        $id = (int)$matches[1];
        $isRestore = isset($matches[2]) && $matches[2] === '/restore';
        $controller = new \Controllers\MemberProgressNoteController();
...
// Dynamic matching for trainer member progress notes endpoints
    ...
    if (preg_match('#^/api/reception/members/([1-9]\d*)/check-out$#', $requestUri, $matches)) {
        AuthMiddleware::hasRole(['super_admin', 'admin', 'reception']);
        if ($method === 'POST') {
            require_once __DIR__ . '/controllers/ReceptionMemberController.php';
            (new \Controllers\ReceptionMemberController())->checkOut((int)$matches[1]);
            $matched = true;
        }
    }

    if (preg_match('#^/api/reception/occupancy$#', $requestUri)) {
        AuthMiddleware::hasRole(['super_admin', 'admin', 'reception']);
        if ($method === 'GET') {
            require_once __DIR__ . '/controllers/ReceptionMemberController.php';
            (new \Controllers\ReceptionMemberController())->occupancy();
            $matched = true;
        }
    }

    if (preg_match('#^/api/reception/members/([1-9]\d*)/renew$#', $requestUri, $matches)) {
        AuthMiddleware::hasRole(['super_admin', 'admin', 'reception']);
        if ($method === 'POST') {
            require_once __DIR__ . '/controllers/ReceptionMemberController.php';
            (new \Controllers\ReceptionMemberController())->renew((int)$matches[1]);
            $matched = true;
        }
    }
    ...
    if (preg_match('#^/api/reception/renewal-notifications/materialize$#', $requestUri)) {
        AuthMiddleware::hasRole(['super_admin', 'admin', 'reception']);
        if ($method === 'POST') {
            require_once __DIR__ . '/controllers/RenewalNotificationMaterializerController.php';
            (new \Controllers\RenewalNotificationMaterializerController())->materialize();
            $matched = true;
        }
    }
    ...
    if (preg_match('#^/api/trainer/members/([1-9]\d*)/measurements$#', $requestUri, $matches)) {
        AuthMiddleware::hasRole(['trainer']);
        $memberId = (int)$matches[1];
        $controller = new \Controllers\TrainerMemberMeasurementController();
        if ($method === 'GET') {
            $controller->index($memberId);
            $matched = true;
        } elseif ($method === 'POST') {
    ...
    if (preg_match('#^/api/trainer/member-measurements/([1-9]\d*)$#', $requestUri, $matches)) {
        AuthMiddleware::hasRole(['trainer']);
        $id = (int)$matches[1];
        $controller = new \Controllers\TrainerMemberMeasurementController();
        if ($method === 'GET') {
    ...
    if (preg_match('#^/api/trainer/member-measurements/([1-9]\d*)/restore$#', $requestUri, $matches)) {
        AuthMiddleware::hasRole(['trainer']);
        if ($method === 'POST') {
            (new \Controllers\TrainerMemberMeasurementController())->restore((int)$matches[1]);
            $matched = true;
        }
    }
    ...
    if (preg_match('#^/api/admin/appointments/([1-9]\d*)/cancel$#', $requestUri, $matches)) {
        AuthMiddleware::handle();
        AuthMiddleware::hasRole(['super_admin', 'admin']);
        if ($method === 'PATCH') {
            require_once __DIR__ . '/controllers/AppointmentController.php';
            (new \Controllers\AppointmentController())->cancelAdminAppointment((int)$matches[1]);
            $matched = true;
        }
    }

    if (preg_match('#^/api/reception/appointments/([1-9]\d*)/cancel$#', $requestUri, $matches)) {
        AuthMiddleware::handle();
        AuthMiddleware::hasRole(['super_admin', 'admin', 'reception']);
        if ($method === 'PATCH') {
            require_once __DIR__ . '/controllers/AppointmentController.php';
            (new \Controllers\AppointmentController())->cancelReceptionAppointment((int)$matches[1]);
            $matched = true;
        }
    }
if (preg_match('#^/api/admin/appointments/([1-9]\d*)/reschedule$#', $requestUri, $matches)) {
    ...
    if ($requestUri === '/api/member/overview' && $method === 'GET') {
        ...
        (new \Controllers\MemberPortalController())->getOverview();
        $matched = true;
    }
