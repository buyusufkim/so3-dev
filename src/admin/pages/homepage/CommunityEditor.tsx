...
export function CommunityEditor({ onClose, onSaved }: { onClose: () => void, onSaved?: () => void }) {
  ...
  const fetchContent = async () => {
    try {
      const res = await apiClient.get('/api/admin/homepage/sections/community/content');
      setData(res.content);
    } catch (err: unknown) {
  ...
  const handleChange = <K extends keyof CommunityContent>(field: K, value: CommunityContent[K]) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      setError(null);
      await apiClient.patch('/api/admin/homepage/sections/community/content', {
        content: data
      });
      setIsDirty(false);
      if (onSaved) onSaved();
    ...
    <div className="fixed inset-0 z-40 bg-black/80 flex items-center justify-center p-4">
      <div className="bg-[#111] border border-white/10 rounded-xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div>
            <h2 className="text-xl font-bold text-white">Topluluk Bölümü Sunum Metinleri</h2>
            <p className="text-sm text-white/50 mt-1">Ana sayfa topluluk alanı başlıklarını ve etiketlerini düzenleyin.</p>
          </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-[#851C35] transition-colors min-h-[100px]"
                  />
                  <p className="text-xs text-white/40 mt-1">Zorunlu değildir. Boş bırakırsanız açıklama paragrafı gizlenir.</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-white/70 mb-1">Tüm Etkinlikler Buton Metni (CTA)</label>
                  <input
                    type="text"
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-[#851C35] transition-colors"
                  />
                  <p className="text-xs text-white/40 mt-1">Zorunlu değildir. Boş bırakırsanız tüm etkinlikler butonu gizlenir.</p>
                </div>
              </div>
            </div>
          )}
