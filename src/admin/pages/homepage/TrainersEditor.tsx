...
export function TrainersEditor({ onClose, onSaved }: { onClose: () => void, onSaved?: () => void }) {
  const [data, setData] = useState<TrainersContent | null>(null);
  ...
  const handleChange = <K extends keyof TrainersContent>(field: K, value: TrainersContent[K]) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      setError(null);
      await apiClient.patch('/api/admin/homepage/sections/trainers/content', {
        content: data
      });
      setIsDirty(false);
      if (onSaved) onSaved();
    ...
    <div className="fixed inset-0 z-40 bg-black/80 flex items-center justify-center p-4">
      <div className="bg-[#111] border border-white/10 rounded-xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div>
            <h2 className="text-xl font-bold text-white">Eğitmenler Bölümü Sunum Metinleri</h2>
            <p className="text-sm text-white/50 mt-1">Ana sayfa eğitmenler alanı başlıklarını düzenleyin.</p>
          </div>
