export interface TrainerTrainingProgramListItem {
  id: number;
  uuid: string;
  title: string;
...
export interface TrainerTrainingProgramsResponse {
  items: TrainerTrainingProgramListItem[];
  pagination: Pagination;
}

export interface TrainerTrainingProgramDetail {
  id: number;
  uuid: string;
  title: string;
  status: "draft" | "active" | "archived";
...
export interface TrainerTrainingProgramCreateResponse {
  id: number;
  uuid: string;
}
...
export interface TrainerProgramExerciseCreateResponse {
  id: number;
  program_id: number;
}
...
export function isTrainerTrainingProgramListItem(item: unknown): item is TrainerTrainingProgramListItem {
...
export function isTrainerTrainingProgramsResponse(res: unknown): res is TrainerTrainingProgramsResponse {
  if (!isObject(res)) return false;
  if (!Array.isArray(res.items)) return false;
  if (!res.items.every(isTrainerTrainingProgramListItem)) return false;
...
export function isTrainerTrainingProgramDetail(detail: unknown): detail is TrainerTrainingProgramDetail {
...
export function isTrainerTrainingProgramCreateResponse(res: unknown): res is TrainerTrainingProgramCreateResponse {
  if (!isObject(res)) return false;
...
export function isTrainerProgramExerciseArray(val: unknown): val is TrainerProgramExercise[] {
  if (!Array.isArray(val)) return false;
  return val.every(isTrainerProgramExercise);
}

export function isTrainerProgramExerciseCreateResponse(val: unknown): val is TrainerProgramExerciseCreateResponse {
  if (!isObject(val)) return false;
  if (!isPosInt(val.id)) return false;
  if (!isPosInt(val.program_id)) return false;
  return true;
