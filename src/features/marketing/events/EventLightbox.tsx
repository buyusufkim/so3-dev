...
export function EventLightbox({ isOpen, onClose, images, title, initialIndex = 0 }: EventLightboxProps) {
  const [currentIndex, setCurrentIndex] = useState(initialIndex);
  const dialogRef = useRef<HTMLDialogElement>(null);

  // Reset state when title or images change, or when opened
  useEffect(() => {
    if (isOpen) {
      setCurrentIndex(initialIndex);
    }
  }, [title, images, isOpen, initialIndex]);
      ...
      <div className="relative w-full h-full max-h-[85vh] flex items-center justify-center p-4 md:p-12" onClick={handleBackdropClick}>
        {currentImage ? (
          <img 
            ...
            alt={currentImage.alt || `${title} - Görsel ${safeIndex + 1}`} 
