...
export function V31Discovery() {
  return (
    <section id="kesfet" className="py-20 md:py-32 bg-[#0A0A0A] overflow-hidden">
      <div className="container mx-auto px-6 lg:px-12 mb-12 md:mb-16">
        <div className="flex flex-col gap-3 max-w-2xl">
          ...
          <p className="text-base md:text-lg text-white/60 font-medium max-w-lg mt-2">
            Kişiye özel antrenmanın gerçekleştiği alanları ve stüdyo atmosferini yakından gör.
          </p>
        </div>
      </div>
        ...
        {DISCOVERY_ITEMS.map((item, index) => (
          <div key={item.id} className={`flex-none ${item.width} snap-center group cursor-pointer relative`}>
            <div className="rounded-lg overflow-hidden border border-white/10 relative">
              <V31MediaPlaceholder label={item.title} aspectRatio={item.aspect} className="w-full transition-transform duration-700 group-hover:scale-105" />
