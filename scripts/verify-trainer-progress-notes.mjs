...
function verify() {
    ...
    const mutationCommitAnchors = [
        { name: 'store', commitAnchor: 'INSERT INTO member_progress_notes' },
        { name: 'update', commitAnchor: 'UPDATE member_progress_notes' },
        { name: 'destroy', commitAnchor: 'UPDATE member_progress_notes' },
        { name: 'restore', commitAnchor: 'UPDATE member_progress_notes' }
    ];
    ...
    for (const { name: mName, commitAnchor } of mutationCommitAnchors) {
        ...
        if (anchorIdx === -1) {
            console.error(`❌ Invariant 13: Missing mutation anchor '${commitAnchor}' in ${mName}`);
            auditAllValid = false;
            continue;
        }
        ...
        if (mutationCommitIdx === -1 || auditIdx < mutationCommitIdx) {
            console.error(`❌ Invariant 13: Audit call in ${mName} is not post-mutation-commit`);
            auditAllValid = false;
            continue;
        }
        ...
        if (innerTryIdx === -1 || innerTryIdx < mutationCommitIdx) {
            console.error(`❌ Invariant 13: Audit call in ${mName} is not enclosed in a dedicated inner try block after commit`);
            auditAllValid = false;
            continue;
        }
        ...
        if (tryOpenBrace === -1 || tryOpenBrace > auditIdx) {
            console.error(`❌ Invariant 13: Invalid try block syntax before audit in ${mName}`);
            auditAllValid = false;
            continue;
        }
        ...
        if (tryCloseBrace === -1 || auditIdx >= tryCloseBrace) {
            console.error(`❌ Invariant 13: Audit call in ${mName} is outside its inner try block`);
            auditAllValid = false;
            continue;
        }
        ...
        if (!catchMatch) {
            console.error(`❌ Invariant 13: Dedicated inner catch (Throwable) block missing after audit try in ${mName}`);
            auditAllValid = false;
            continue;
        }
    }
    ...
    for (const { name: mName } of mutationCommitAnchors) {
        const mBody = methods[mName];
